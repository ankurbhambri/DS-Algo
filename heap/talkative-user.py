...
You are given a list of tuples where each tuple represents a chat message in the form (username, word_count). 

Each tuple indicates the number of words spoken by a user in a single message. Your task is to identify the top N most talkative users based on their total word counts.

Write a function TalkTalkativeUsers that takes:

1. A list of tuples user_word_counts where each tuple is (username, word_count).
2. An integer N, the number of most talkative users to return.
...
- The number of unique users k could be very large.
...
# TC: O(m + k log n) - where m is the length of user_word_counts, k unique users and n is the size of the heap.
def TalkTalkativeUsers(user_word_counts, k):
    ...
    for user, word_count in total_word_counts.items():  # O(k), where k is the number of unique users
        heapq.heappush(heap, (word_count, user))  # # O(log n) Push the current user into the heap
        if len(heap) > k:  # If the heap exceeds size n, remove the smallest element
...
user_word_counts = [
    ('John', 1), ('Maria', 1), ('John', 5), ('Maria', 2), 
    ('Alex', 10), ('John', 2), ('Alex', 4), ('Bob', 12),
    ('Charlie', 8), ('Daisy', 3), ('Eva', 15), ('Frank', 2)
]
k = 3
print(TalkTalkativeUsers(user_word_counts, k))