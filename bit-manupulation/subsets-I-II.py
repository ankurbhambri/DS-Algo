...
# Backtracking approach (preferred)
...
# Another approach using bit manipulation (not preferred)

# TC: O(n * 2^n)
# SC: O(n * 2^n) for storing all subsets
...
# Tc: O(n * 2^n) -> [O(n log n + n * 2^n) for sorting + backtracking, but 2^n  >>  log n, the dominant term is: O(n * 2^n)]
# SC: O(n) + O(2^n * n)
