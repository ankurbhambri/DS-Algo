...
def func(n, parent, jump):
    maxbit = 18  # Maximum bits for binary lifting

    # Mark which nodes are occupied
    occupied = [False] * (n + 1)  # Using 1-based index

    # Initialize dp array for binary lifting (maxbit x n+1)
    dp = [[-1] * (n + 1) for _ in range(maxbit)]
    ...
    def calc(node):
        ...
        for i in range(maxbit - 1, -1, -1):  # Traverse binary lifting table
            jp = dp[i][curr]
